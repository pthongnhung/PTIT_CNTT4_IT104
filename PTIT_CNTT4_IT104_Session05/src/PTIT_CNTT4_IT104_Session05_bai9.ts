class $Book {
    ...
    public update(title: string, author: string, year?: number): void {
        if (title && title.length > 0) this.title = title;
        if (author && author.length > 0) this.author = author;
        if (typeof year === "number" && year > 0) this.year = year;
    }
}
...
class $Library {
    ...
    public updateBookById(id: number, newTitle: string, newAuthor: string, newYear?: number): boolean {
        for (let i = 0; i < this.books.length; i++) {
    ...
    public deleteBookById(id: number): boolean {
        for (let i = 0; i < this.books.length; i++) {
            if (this.books[i].getId() === id) {
